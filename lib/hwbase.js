...
/**
 * @typedef {import('../types').HwCompBase<HwAppBase> } HwCompBase
 * @typedef {import('../types').HwPluginBase<HwAppBase>} HwPluginBase
...
class HwAppBase extends EventEmitter {
  ...
  constructor() {
    ...
    program
      .addOption(
        new Option(
          "-e, --env <env>",
          'app node run env. "development" | "production" | [other custom name]',
        ).default("development"),
      )
      ...
      .addOption(
        new Option(
          "-m, --use_ms_parame",
          "Use micro service parame, ignore config.json",
        ).default(false),
      )
      .addOption(
        new Option("--ms_disable", "Disable micro service").default(false),
      )
      .addOption(
        new Option("--ms_ip <ip>", "The service use IP").conflicts(
          "ms_disable",
        ),
      )
      .addOption(
        new Option("--ms_port <number>", "The service port number")
          .conflicts("ms_disable")
          .argParser(HwAppBase.#cmdParsePort),
      )
      .addOption(
        new Option("--ms_prefix <ip>", "service router prefix").default(""),
      )
  ...
  /** 返回 UUID, instanceId
   *
   * @returns {string} - UUID, instanceId
   */
  get uuid() {
   ...
   * @readonly
   * @memberof HwAppBase
   * @returns {import('../types').HwAppCmdOpt} - 命令行参数
   */
  get cmdOpts() {
  ...
  async getConfig(cfgName) {
    ...
    if (
      this._cfg?.mservice?.enable === true &&
      ...
      if (
        ncfg.dependencies &&
        ncfg.dependencies instanceof Array &&
        ncfg.dependencies.indexOf(cfgName) >= 0
      ) {
      ...
      if (cfgRet === null) {
        this.#_logger.error(`Can't find config ${cfgName} from rfinder`);
      }
        ...
        if (packPath === null) {
          ...
          switch (packPath) {
            case "@heywoogames/hw-redis":
              packPath = Path.normalize(
                Path.join(__dirname, "../plugins/hw-redis.js"),
              );
              break;
            case "@heywoogames/hw-mq":
              packPath = Path.normalize(
                Path.join(__dirname, "../plugins/hw-mq.js"),
              );
              break;
          }
        }
  ...
  async init() {
    ...
    if (
      typeof this._cfg?.mservice?.ip !== "string" ||
      this._cfg.mservice.ip.length < 8
    ) {
      this._cfg.mservice.ip = nnet.localIp();
    }

    if (
      typeof this.#_cmdOpts.node_name === "string" &&
      this.#_cmdOpts.node_name.length > 0
    ) {
    ...
    if (this._cfg?.cfgRedis?.enable === true) {
      if (
        typeof this._cfg.cfgRedis.key !== "string" ||
        typeof this._cfg.cfgRedis.mqAlias !== "string" ||
        typeof this._cfg.cfgRedis.rdAlias !== "string"
      ) {
        this.logger.error(
          "xxx key,mqAlias,rdAlias need by cfgRedis in config.json",
        );
        process.exit(2);
      }
    } else {
    ...
    if (this._cfg?.cfgRedis?.enable === true) {
      ...
      if (mqIns === null) {
        this.logger.error(
          `cfgRedis.mqAlias[ ${this._cfg.cfgRedis.mqAlias} ] not found in plugins`,
        );
        process.exit(3);
      }
      ...
      if (rdIns === null) {
        this.logger.error(
          `cfgRedis.rdAlias[ ${this._cfg.cfgRedis.rdAlias} ] not found in plugins`,
        );
        process.exit(3);
      }
  ...
  async #loadCfgFromRedis(channel, msg) {
    ...
    const cfgT = await this.#loadCfgFromRedisRaw();
    if (cfgT !== null && this._cfg.cfgRedis) {
      this.emit(
        "cfg_change",
        this._cfg.cfgRedis.key,
        cfgT,
        this._cfg.cfgRedis.key,
      );
    }
  ...
  async #loadComps() {
    const compsPath =
      this.env.PROJ_PATH + "/" + (this._cfg?.compsPath ?? "comps");
    ...
    for (let it of files) {
      ...
      try {
        ...
        ins._filePath = Path.relative(
          this.env.PROJ_PATH,
          `${compsPath}/${Path.parse(it).name}`,
        );
  ...
  async start() {
    ...
    if (this._ms?._finder) {
      await this._ms._finder.onAfterStartAll();
    }

    // 如果有redis插件
    if( this._pluginsMap['_redis'] && this._cfg?.serviceName && this._cfg.serviceName.length > 2 ) {
      /** @type { import('ioredis').Redis } */
      // @ts-ignore
      const rdM = this._pluginsMap['_redis']?.getInsByName("master");
      if( rdM !== null ) {
        rdM.hset("hw:qt:services", this._cfg.serviceName,`${this._cfg.mservice.ip}:${this._cfg.mservice.port}`)
      }
    }
   ...
   */
  getDstPath() {
    let dtsPath = Path.join(env.PROJ_PATH, "index.d.ts");
    if (fs.existsSync(dtsPath) === false) {
      dtsPath = Path.join(env.PROJ_PATH, "main.d.ts");
      if (fs.existsSync(dtsPath) === false) {
        return "";
      }
    }
   ...
   */
  mountComps() {
    if (this.#hasMountComp === true) {
      return;
    }

    this.#hasMountComp = true;
    for (let name in this.#_compmentMap) {
      // @ts-ignore
      if (this[name]) {
        this.logger.error(
          `comp[ ${name} ] already exists, can not mount! Please change the name of the component.`,
        );
        process.exit(1);
      }
      // @ts-ignore
    ...
    let hasDb = false;
    if (this.#_cmdOpts.api_doc === true) {
      let dtsPath = this.getDstPath();
      let findDts = dtsPath.length > 0;
      ...
      if (findDts === true) {
        ...
        for (let name in this.#_compmentMap) {
          const ins = this.#_compmentMap[name];
          const className = ins.constructor.name;

          compImport.push(
            `import { ${className} } from "${ins._filePath.replace(/\\/g, "/")}";`,
          );
          compExport.push(`  ${className},`);
          compDef.push(`    ${name}: InstanceType<typeof ${className}>;`);
        }

        // 生成组件 导入
        this.insertDtsContent(dtsPath, "comp_import", compImport.join("\n"));

        // 生成组件 导出
        this.insertDtsContent(dtsPath, "comp_export", compExport.join("\n"));

        // 生成组件定义
        this.insertDtsContent(dtsPath, "comp_def", compDef.join("\n"));

        /////////// 生成插件的
        const pluginImport = [];
        const pluginExport = [];
        const pluginDef = [];
        let hasRedisOrMq = false;

        for (let name in this._pluginsMap) {
          const ins = this._pluginsMap[name];
          if (ins._info?.enable !== true) {
            continue;
          }

          const className = ins.constructor.name;
          if (
            ins._info?.package === "@heywoogames/hw-redis" ||
            ins._info.package === "@heywoogames/hw-mq"
          ) {
            hasRedisOrMq = true;
          } else if (ins._info.package === "@heywoogames/hw-db") {
            hasDb = true;
          } else {
            if (ins._info.package) {
              pluginImport.push(
                `import { ${className} } from "${ins._info.package.replace(/\\/g, "/")}";`,
              );
            } else if (ins._info.path) {
              pluginImport.push(
                `import { ${className} } from "${Path.relative(env.PROJ_PATH, ins._info?.path).replace(/\\/g, "/")}";`,
              );
            }
          }

          pluginExport.push(`  ${className},`);
          pluginDef.push(`    ${ins._alias ?? "_" + ins.name}: ${className};`);
        }

        if (hasRedisOrMq === true) {
          pluginImport.unshift(`import type { Redis } from "ioredis";`);
          pluginImport.unshift(
            `import { HwRedisCli, HwMqCli } from "@heywoogames/hw-base";`,
          );
        }

        if (hasDb === true) {
          pluginImport.unshift(
            `import { HwDbCli, DBFree } from "@heywoogames/hw-db";`,
          );
          pluginImport.unshift(
            `import { Sequelize, Model } from "@sequelize/core";`,
          );
          pluginExport.unshift(`  Sequelize,`);
          pluginExport.unshift(`  Model,`);
        }
        ...
        pluginImport.unshift(
          `import type { HwResponse, HwRespPageData, HwResponseSimple } from "@heywoogames/qt-types";`,
        );
        pluginImport.unshift(
          `import type {
  HwMsResp,
  ...
  HwExpressResponse,
  Redis
} from "@heywoogames/hw-base";`,
        );

        pluginExport.unshift(`  HwMsResp,`);
        pluginExport.unshift(`  HwMSServiceBase,`);
        pluginExport.unshift(`  HwMicroService,`);
        pluginExport.unshift(`  Redis,`);
        pluginExport.unshift(`  HwExpressRequest,`);
        pluginExport.unshift(`  HwExpressResponse,`);
        pluginExport.unshift(`  HwResponse,`);
        pluginExport.unshift(`  HwRespPageData,`);
        pluginExport.unshift(`  HwResponseSimple,`);

        // 生成组件 导入
        this.insertDtsContent(
          dtsPath,
          "plugin_import",
          pluginImport.join("\n"),
        );
        ...
        // 生成组件定义
        this.insertDtsContent(dtsPath, "plugin_def", pluginDef.join("\n"));

        //
        this.logger.info(`-- typedef plugin and comp update success`);
      }
    }
  }
   ...
   *  - 开始标记: `@s_${flag}`
   *  - 结束标记: `@e_${flag}`
   *  - 如果同时找到开始/结束标记才更改，否则退出函数
   *  - 开始标记 和 结束标记 之间的内容会被替换为新的文本
   *  - 开始/结束标记 可以出现在一行的任意位置，两个标记不能在同一行
   *
   * @param {string} filePath 文件路径
   * @param {string} flag 行标志
   * @param {string} content 插入内容
   */
  insertDtsContent(filePath, flag, content) {
    try {
      const startLineFlag = `@s_${flag}`;
      const endLineFlag = `@e_${flag}`;
      ...
      for (let i = 0; i < lines.length; i++) {
        if (lines[i].includes(startLineFlag)) {
          startLineIndex = i;
        }
        if (lines[i].includes(endLineFlag)) {
          endLineIndex = i;
          break;
        }
      }
      ...
      if (startLineIndex === -1 || endLineIndex === -1) {
        console.log(`未找到标记: ${startLineFlag} 或 ${endLineFlag}`);
        return;
      }

      // 如果开始标记在结束标记之后，则退出函数
      if (startLineIndex >= endLineIndex) {
        console.log("开始标记不能在结束标记之后");
        return;
      }

      // 构建新的文件内容
      const newLines = [
        ...lines.slice(0, startLineIndex + 1), // 开始标记之前的行（包含开始标记行）
        content, // 新内容
        ...lines.slice(endLineIndex), // 结束标记及其之后的行
      ];

      // 将新内容写回文件
      fs.writeFileSync(filePath, newLines.join("\n"), "utf8");
    } catch (/**@type {any}*/ err) {
      console.error(`file ${filePath} update error: ${err.message}`);
    }
  }
}
