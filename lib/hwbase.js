...
class HwAppBase extends EventEmitter {
  ...
  constructor() {
    ...
    program
      ...
      .addOption(
        new Option("-m, --use_ms_parame", "Use micro service parame, ignore config.json").default(
          false,
        ),
      )
  ...
  async #loadComps() {
    ...
    for (let it of files) {
      ...
      try {
        ...
        ins._filePath = Path.relative(this.env.PROJ_PATH, `${compsPath}/${Path.parse(it).name}`);
  ...
  async start() {
    ...
    if (this._pluginsMap["_redis"] && this._cfg?.serviceName && this._cfg.serviceName.length > 2) {
      ...
      if (rdM !== null) {
        rdM.hset(
          "hw:qt:services",
          this._cfg.serviceName,
          `${this._cfg.mservice.ip}:${this._cfg.mservice.port}`,
        );
      }
    }
    ...
    if (this.#_cmdOpts.api_doc === true) {
      ...
      if (findDts === true) {
        ...
        for (let name in this.#_compmentMap) {
          ...
          compImport.push(`import { ${className} } from "${ins._filePath.replace(/\\/g, "/")}";`);
        ...
        if (hasRedisOrMq === true) {
          ...
          pluginImport.unshift(`import { HwRedisCli, HwMqCli } from "@heywoogames/hw-base";`);
        }

        if (hasDb === true) {
          pluginImport.unshift(`import { HwDbCli, DBFree } from "@heywoogames/hw-db";`);
          pluginImport.unshift(`import { Sequelize, Model } from "@sequelize/core";`);
        ...
        pluginImport.unshift(`import { RequestHandler, RequestParamHandler } from "express";`);
  ...
  HwExpressResponse
        ...
        // 生成组件 导入
        this.insertDtsContent(dtsPath, "plugin_import", pluginImport.join("\n"));

        // 生成组件 导出
        this.insertDtsContent(dtsPath, "plugin_export", pluginExport.join("\n"));
