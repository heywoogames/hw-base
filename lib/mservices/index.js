...
function loadServices(ins, serPath) {
  ...
  for (let it of files) {
    let serviceClass = require(`${servicePath}/${it}`);
    if (serviceClass.nserviceDefault) {
      serviceClass = serviceClass.nserviceDefault;
    }

    try {
      const service = new serviceClass(ins);
      ins._services[`${servicePath}/${it}`] = service;
      ins._app.logger.info(`service [${it}] init success`);
    } catch (err) {
      ins._app.logger.error(`service [${it}] init err:`, err);
      process.exit(1);
    }
  }
}
...
class HwMicroService {
  /**
   *
   * @param {HwAppBase} app - app instance
   */
  constructor(app) {
    /** @type {HwAppBase} */
    this._app = app;

    /** @type { Record<string,import('../../types').HwMSServiceBase<HwAppBase>> } */
    this._services = {};
    ...
    if (this._app.cmdOpts.api_doc === true) {
      ...
      if (packageJson.apiGen?.apis instanceof Array) {
        ...
        this._app.logger.warn(
          "/api-docs error: package.json apiGen.apis is not an array",
        );
        return;
      }
      ...
      if (
        packageJson.apiGen?.allowedTags instanceof Array &&
        packageJson.apiGen.allowedTags.length > 0
      ) {
  ...
  `,
      ...
      this._app.logger.warn(
        `api-docs - http://127.0.0.1:${this._cfg.port}/api-docs`,
      );
    }

    const servicePath =
      this._app.env.PROJ_PATH + "/" + (this._cfg.path ?? "mservices");

    loadServices(this, servicePath);

    if (this._cfg?.finder?.enable === true) {
      this._finder = new HwFinder(this._app);
      await this._finder.init(this._cfg, /**@type {any}*/ (this._app.logger));
    }
  }
  ...
  async start() {
    this._eapp.listen(this._cfg.port, () => {
      this._app.logger.info(
        `hw-mservice app listening on port ${this._cfg.port}`,
      );
    });
    ...
    this._eapp.get("/echo", (req, res) => {
      res.sendHw(
        0,
        Object.keys(req.query).length > 0 ? req.query : { null: null },
      );
    });
   ...
   * @param {boolean} [selOne= true] - 是否选择一个,选择权重最高的那一个
   * @returns {Promise<string | string[] | null>} - 服务地址
   */
  async getService(serviceName, selOne = true) {
    if (this._finder === null) {
      return null;
    }
    const hosts = await this._finder.getService(serviceName);
    if (!selOne) {
      /** @type {string[]} */
      let rets = [];
      for (let it of hosts) {
        rets.push(`${it.ip}:${it.port}`);
      }
    ...
    if (hosts.length === 0) {
      ...
      let idx = utils.getAreaByProbability(weights);
      let host = hosts[idx];
      return `${host.ip}:${host.port}`;
    }
  }
   ...
   * @returns {Promise<void>} - 消息发布数量
   */
  async publish(channel, message) {
    if (this._finder._finders === null) {
      this._app.logger.warn("Finder not create!");
      return;
    }

    return this._finder._finders.publish(channel, message);
  }
} // HwMicroService
