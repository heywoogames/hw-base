...
class RedisNamingCli {
   ...
   * @param {number} port - 实例端口
   * @returns {boolean}
   */
  #isSelf ( ip, port ) {
    return ip === this._finder._nameIns.ip && port === this._finder._nameIns.port;
  }
  ...
  async destroy () {
    try{
      ...
      this._logger.error( 'Error destroying RedisNamingCli:', err.toString() );
    }
  }
