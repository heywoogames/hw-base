...
class RedisNamingCli {
  ...
  async registerInstance() {
    ...
    await this._finder._rd
      .pipeline()
      .hset(
        this._finder.groupKey,
        this._finder.insName,
        JSON.stringify(nameIns),
      )
    ...
    if (subs && subs instanceof Array) {
      ...
      for (let it of subs) {
        ...
        this._finder.subscribe(
          `${this._finder.groupKey}:${it.serviceName}`,
          (channel, msg) => {
            try {
  ...
  async getAllInstancesFromRedis(serviceName) {
    ...
    const res = [];
    const instances = await this._finder._rd.hmget(
      this._finder.groupKey,
      ...fields,
    );
  ...
  async getAllInstances(serviceName) {
    ...
    hosts = hosts.filter((v) => {
      return (
        v.healthy !== false &&
        v.enabled !== false &&
        v.metadata.updateTm > expireTime
      );
    });
        ...
        case "up":
          {
            ...
            if (hosts) {
              const idx = hosts.findIndex(
                (v) => v.ip === info.ip && v.port === info.port,
              );
        ...
        case "stat":
          {
            ...
            if (hosts) {
              const v = hosts.find(
                (v) => v.ip === message.info.ip && v.port === message.info.port,
              );
              if (v) {
                if (typeof info.weight === "number") {
  ...
  async onAfterStartAll() {
    if (this._serviceCache.size > 0) {
      for (let it of this._serviceCache.keys()) {
        this._finder._app.emit(
          "services_change",
          it,
          this._serviceCache.get(it) ?? [],
        );
      }
    }
  }
