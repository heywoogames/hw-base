 ...
 * 配置环境变量解析工具
 *
 * 当配置的 host / password / port 字段的值以 "ENV_HW_" 开头时，
...
/**
 * 解析 Redis / MQ redis 配置中的 host、password 和 port
 * @param {Partial<{ host?: string, password?: string, port?: number | string }>} cfg - 配置对象（HwRedisCfg 或 MQ redis config）
...
function resolveEnvConfig(cfg) {
  ...
  if (typeof cfg.port === "string" && cfg.port.startsWith("ENV_HW_")) {
    const envVal = process.env[cfg.port];
    if (envVal !== undefined) {
